...
from tronpy.keys import to_base58check_address, is_address

from app.domain.tron.core.repositories import TronRepository
from app.domain.tron.core.schemas import TronCredentials, TronDetails, TronListRequestSchema, TronDetailsFull
...
class TronCredentialsQuery:
    ...
    def get_address_info(self, credentials: TronCredentials) -> TronDetails:
        try:
        ...
        except AddressNotFound: # Добавлено для того, чтобы смотреть пустые кошельки (без этого вылезает ошибка, что кошелек не найден)
            return TronDetails(address=credentials.address, balance_trx=Decimal(0), bandwidth=0, energy=0)
...
class TronListQuery:
    ...
    async def __call__(
        self, payload: TronListRequestSchema
    ) -> Paginated[TronDetailsFull]:
        return await self.repository.get_tron(
